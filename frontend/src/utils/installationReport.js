...
export async function generateInstallationReport({ router, stats, logs, gpsLocation, technician }) {
  ...
  addInfoRow('Installation Date:', installDate.toLocaleString('en-US', {
    month: 'short',
    day: 'numeric', 
    year: 'numeric',
    hour: '2-digit',
  ...
  addInfoRow('Captured:', new Date(gpsLocation.timestamp).toLocaleString('en-US', {
    month: 'short',
    day: 'numeric',
    year: 'numeric', 
    hour: '2-digit',
  ...
  const lastSeenText = router.last_seen 
    ? new Date(router.last_seen).toLocaleString('en-US', {
        month: 'short',
        day: 'numeric',
        year: 'numeric',
        hour: '2-digit',
  ...
  doc.text('Generated by VacatAd Router Management System', pageWidth / 2, y + 3, { align: 'center' });
  ...
  doc.text(`Report generated: ${timestamp}`, pageWidth / 2, y + 8, { align: 'center' });
  
  // Document ID in bottom right
  doc.setFontSize(7);
  doc.setTextColor(...colors.neutralLight);
  doc.text(`Doc ID: ${router.router_id}-${Date.now()}`, pageWidth - margin, y + 8, { align: 'right' });
