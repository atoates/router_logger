# Documentation Index

This folder contains the up-to-date operational docs and guides.

## Start here

- **Quickstart**: `../QUICKSTART.md`
- **Environment variables**: `ENVIRONMENT-VARIABLES.md`

## Operations

- **Deployment checklist**: `ops/RAILWAY-DEPLOYMENT-CHECKLIST.md`
- **Railway mobile setup**: `ops/RAILWAY-MOBILE-SETUP.md`
- **Selective deployments**: `ops/RAILWAY-SELECTIVE-DEPLOYMENT.md`
- **Migration status**: `ops/MIGRATION-STATUS.md`
- **Date sync automation**: `ops/DATE-SYNC-AUTOMATION.md`
- **Sync dates quick start**: `ops/SYNC-DATES-QUICK-START.md`

## Integrations

- **RMS OAuth setup**: `RMS-OAUTH-SETUP.md`
- **RMS configuration guide**: `RMS-CONFIGURATION-GUIDE.md`
- **RMS API integration**: `RMS-API-INTEGRATION.md`
- **MQTT setup**: `MQTT-SETUP-GUIDE.md`
- **MAC address sync**: `MAC-ADDRESS-SYNC.md`

## ClickUp

- **Comment actions**: `clickup/CLICKUP-COMMENT-ACTIONS.md`
- **Sync button implementation**: `clickup/CLICKUP-SYNC-BUTTON-IMPLEMENTATION.md`
- **Realtime status update**: `clickup/REALTIME-CLICKUP-STATUS-UPDATE.md`

## Mobile

- **Mobile app fresh start**: `mobile/MOBILE-APP-FRESH-START.md`
- **Implementation plan**: `mobile/MOBILE-APP-IMPLEMENTATION-PLAN.md`
- **Setup**: `mobile/MOBILE-APP-SETUP.md`

## Architecture & Guides

- **Location tracking architecture**: `architecture/LOCATION-TRACKING-ARCHITECTURE.md`
- **Property search guide**: `guides/PROPERTY-SEARCH-GUIDE.md`

## Archive

- Historical work notes moved from repo root: `archive/work-notes/`
